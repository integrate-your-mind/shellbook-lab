...
@MainActor
final class ShellbookLabStore: ObservableObject {
  ...
  static let defaultBaseURL = "http://127.0.0.1:8791"
  private static let baseURLKey = "ShellbookLabMacBaseURL"
  ...
  func currentSnapshot() -> DashboardSnapshot? {
    if case .loaded(let snapshot) = dashboardState {
      return snapshot
    }
    return nil
  }

  func client() throws -> ShellbookLabClient {
